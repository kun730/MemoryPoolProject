...
CentralCache::CentralCache() {
    ...
    for (auto& lock : m_centralFreeListLocks)
    {
        lock.clear();
    }
...
void* CentralCache::fetchRange(size_t index)
{
    ...
    // 自旋锁保护
    while (m_centralFreeListLocks[index].test_and_set(std::memory_order_acquire))
    {
    ...
    try 
    {
        ...
        if (!result)
        {
            ...
            if (!result)
            {
                m_centralFreeListLocks[index].clear(std::memory_order_release);
                return nullptr;
            }
            ...
            {  // 确保至少有两个块才构建链表
                ...
                *reinterpret_cast<void**>(result) = nullptr;
                ...
                size_t trackerIndex = m_spanCount.load(std::memory_order_relaxed);
                if (trackerIndex < m_spanTrackers.size())
                {
                    m_spanTrackers[trackerIndex].spanAddress.store(start, std::memory_order_release);
                    m_spanTrackers[trackerIndex].numPages.store(numPages, std::memory_order_release);
                    m_spanTrackers[trackerIndex].blockCount.store(blockNum, std::memory_order_release); // 共分配了blockNum个内存块
                    m_spanTrackers[trackerIndex].freeCount.store(blockNum - 1, std::memory_order_release); // 第一个块result已被分配出去，所以初始空闲块数为blockNum - 1
                }
            }
        } 
        else 
...
SpanTracker* CentralCache::getSpanTracker(void* blockAddr)
{
    ...
    {
        void* spanAddr = m_spanTrackers[i].spanAddress.load(std::memory_order_relaxed);
